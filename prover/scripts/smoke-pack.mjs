...
const tempDir = mkdtempSync(join(tmpdir(), "anyware-prover-smoke-"));
...
try {
  ...
  const script = `
    import { AnywareClient, BUNDLE_VERSION, computeMappingSlot } from "anyware-prover";
    const client = new AnywareClient({ ethRpcUrl: "https://source.example", beaconApiUrl: "https://beacon.example" });
  ...
  execFileSync("pnpm", ["exec", "anyware-prover"], { cwd: tempDir, stdio: "inherit" });
